...
<div style="margin: 10px 0">
    ...
    <el-table :data="tableData.slice((currentPage-1)*pageSize,currentPage*pageSize)" stripe border="true" @selection-change="handleSelectionChange">
	  ...
	  <el-table-column prop="recordId" label="上传编号" sortable />
      <el-table-column prop="paperId" label="论文编号" sortable />
      <el-table-column prop="userId" label="读者编号" sortable/>
      <el-table-column prop="dateTime" label="上传时间" sortable/>
  ...
  methods: {
    ...
    load(){
      if(this.search1!=''&this.search2==''){
        request.get("/Uploads/getBySingleFilter",{
          params:{
            key: "paperId",
            value: this.search1,
          }
        ...
        request.get("/Uploads/getBySingleFilter1",{
          params:{
            key: "userId",
            value: this.search2,
          }
      ...
      {
        request.get("/Uploads/getByDoubleFilters",{
          params:{
            key1: " paperId",
            value1: this.search1,
			key2: " userId",
      ...
      request.get("/Uploads/getAll",).then(res =>{
        console.log(res)
		    this.tableData = res.data
		    this.total = res.data.length
		  }).catch(err=>{
    ...
    handleDelete(row){
      ...
      request.get("/Uploads/deleteTuple",{
		  params:{
